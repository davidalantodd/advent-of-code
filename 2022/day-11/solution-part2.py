...
#calculate the least common multiple of all of the divisors
divisors = []
for mon in monkeys:
    divisors.append(int(mon.test))
        
lcm = reduce(lambda a, b: a*b // math.gcd(a,b), divisors)

#iterate for 20 rounds
x = 1
for ra in range(10000):
    # print(monkeys)
    # print(x)
    x+=1
    ...
    for r in range(len(monkeys)):
        ...
        for i in range(len(monkeys[r].items)):
            monkeys[r].items[i] = worry(monkeys[r].items[i], monkeys[r].operation[0], monkeys[r].operation[1])

            #part 2 - handle worry by finding the remainder after the lcm
            monkeys[r].items[i] = int(monkeys[r].items[i]) % lcm
...
print('solution to part 2: ', monkey_business)